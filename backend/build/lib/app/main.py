...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if mcp_tools:
            registered = register_mcp_tools(mcp_tools)
            log.info("MCP tools integrated: registered %d tool(s) into langgraph", registered)
        else:
            log.info("No MCP tools available (either not configured or servers unreachable)")
