...
from app.agent.tools import execute_tool_async, tools_catalog_for_prompt
...
def retrieve_context(state: AgentState) -> Dict[str, Any]:
    ...
    hits = [
        {"text": "your name is nimisha purushan", "source": "doc1.txt"},
        {"text": "your age is 25", "source": "doc2.txt"},
...
class PlanSchema(BaseModel):
    ...
    def model_json_schema(cls, **kwargs):
        ...
        # Ensure all object properties have additionalProperties: false for Azure OpenAI
        if "properties" in schema and "arguments" in schema["properties"]:
...
def plan_action(state: AgentState) -> Dict[str, Any]:
    ...
    llm = get_chat_llm().with_structured_output(PlanSchema, method="function_calling")
    prompt = [
